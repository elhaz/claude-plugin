...
`gpt-image-1.5`(고품질, 기본 hq) / `gpt-image-1-mini`(고속, 기본 fast)를 사용해 이미지를 생성하거나 편집한다. 조직 인증(신분증 촬영) 완료 계정은 `--model v2`로 `gpt-image-2`를 쓸 수 있다. `gpt-image-1`, `gpt-image-2-2026-04-21` 스냅샷도 `--model`에 원시 ID로 지정 가능.
...
| `--model` | `fast`(mini) / `hq`(1.5, 인증 불필요) / `v2`(gpt-image-2, 조직 인증 필요) / 원시 ID | `fast` |
