...
> 2. **Obsidian 보호**: 최종 완성된 파일만 vault로 이동 (Obsidian 자동 수정 방지). 이동 위치는 **같은 채널의 기존 문서 폴더 우선**, 없을 때만 `00_Inbox/` (7단계 참조)
...
### 7단계: 배치 위치 결정 및 최종 파일 이동

재구성이 완료된 최종 파일을 vault의 적절한 위치로 이동한다.

> [!important] 배치 위치는 `00_Inbox/` 가 아니라 **채널 전용 폴더 우선**
> 같은 채널의 영상을 이미 문서화해 둔 폴더가 있으면 그곳이 정답이다. `00_Inbox/` 는 그런 폴더가 없을 때의 폴백이다.
> 이 판단을 건너뛰면 채널별로 축적된 문서 계열에서 이탈한 고아 문서가 된다.

#### 7.0 채널 전용 폴더 탐색 (필수 선행)

4단계에서 얻은 **채널명**으로 vault 안의 기존 문서를 검색한다. 검색에는 Grep 도구를 사용한다 (`--glob "**/*.md"`).

- 검색어: 채널명 (예: `박종훈의 지식한방`). 결과가 없으면 채널명의 핵심 어절(예: 진행자 이름)로 한 번 더 검색한다.
- `.claude/worktrees/` 등 작업본 경로의 히트는 **무시**한다. vault 본체 경로만 후보로 삼는다.

검색 결과에 따라 분기한다:

| 상황 | 배치 위치 |
|------|-----------|
| 동일 채널 문서가 **한 폴더에 모여 있음** | 그 폴더 (`DEST`) |
| 동일 채널 문서가 **여기저기 흩어져 있음** | 가장 많이 모인 폴더. 판단이 애매하면 사용자에게 질의 |
| 동일 채널 문서 **없음** | `00_Inbox/` (기존 동작) |

#### 7.1 채널 전용 폴더를 찾은 경우

세 가지를 함께 처리해야 완료된다. **파일 이동만 하고 끝내지 말 것.**

**① 기존 문서의 구조 컨벤션 맞추기**

같은 폴더의 기존 문서 1개를 Read 하여 문서 구조를 확인하고, 새 문서를 동일한 형태로 보정한다. 자주 나타나는 요소:

- frontmatter 바로 다음, H1 **앞**에 오는 브레드크럼 (`> [[상위폴더 README|상위]] > [[채널인덱스]] > 짧은제목`)
- `## 영상 정보` 항목의 순서와 구성 (`채널` / `언어` / `주제` 등)
- `## 개요` 안의 `> [!summary] 핵심 요약` 콜아웃
- `## 목차` (H2 섹션들에 대한 `[[#섹션명]]` 앵커 링크)

> [!warning] 컨벤션은 폴더마다 다르다
> 위 목록은 예시일 뿐이다. **반드시 실제 기존 문서를 읽고** 그 폴더의 실제 형태를 따를 것. 목록에 있는 요소를 기계적으로 넣지 않는다.
...
폴더 안에 **폴더명과 같은 이름의 마크다운**이 있으면 그것이 채널 인덱스다 (예: `박종훈의 지식한방/박종훈의 지식한방.md`).

- 인덱스의 영상 목록 섹션에 `[[파일명|설명적 제목]]` 형식으로 링크를 추가한다
- 목록이 `### YYYY년 M월` 같은 시기별 소제목으로 나뉘어 있으면 해당 시기 섹션에 넣는다. 섹션이 없으면 **새로 만들어 최신순 위치**에 배치한다
- 별칭(`|` 뒤)은 기존 항목들의 서술 방식을 따른다 (보통 `핵심주제: 부연설명`)

> [!note] Base 자동집계 폴더는 예외
> 대상 폴더가 `.base` 뷰로 문서를 자동집계하는 구조라면 인덱스에 하드코딩 링크를 추가하지 않는다. 폴더 내 `.base` 파일이나 인덱스 문서의 인라인 base 블록 유무로 판단한다.
...
> - **이미지 추출 없음**: 이동 후 작업폴더 즉시 삭제
> - **이미지 추출 있음**: **작업폴더 유지** (영상 파일 보관), 사용자 확인 후 삭제

```bash
# 이미지 추출이 없는 경우에만 즉시 정리
rm -rf "$WORK_DIR"
```
...
- 생성된 파일 경로
- 파일 제목
- **최종 배치 경로**와 그 위치를 고른 이유 (채널 전용 폴더 발견 여부)
- 채널 인덱스에 링크를 추가했다면 그 사실
- 재구성된 섹션 구조
- 추가 작업 안내 (`00_Inbox/` 폴백이었다면 적절한 PARA 폴더로의 이동 제안 등)
...
- prompt: |
    ...
    6. 배치 위치 결정 — 채널명으로 vault를 검색해 같은 채널의 기존 문서 폴더가 있으면
       그곳으로 이동하고, 기존 문서의 구조 컨벤션을 따르며, 채널 인덱스 문서에 링크를 추가한다.
       없으면 00_Inbox/로 이동한다.
    7. 완료 시 파일명과 최종 배치 경로 보고
...
2. **최종 파일만 이동**: 재구성 완료 후 채널 전용 폴더(없으면 `00_Inbox/`)로 이동
3. **채널 폴더 우선 배치**: 같은 채널 문서가 모인 폴더가 있으면 그곳에 배치하고, 기존 문서의 구조 컨벤션을 따르며, 채널 인덱스에 링크 추가
4. **VTT 삭제**: 변환 완료 후 원본 VTT 파일은 자동 삭제
5. **재구성 필수**: 단순 변환이 아닌 Obsidian 스타일 재구성까지 완료해야 함
6. **내용 이해**: 자막 내용을 이해하고 의미 있는 섹션으로 구조화
7. **링크 연결**: 주요 인물/기관/개념에 내부 링크 적용
8. **병렬 처리**: 여러 URL은 서브에이전트로 병렬 처리
9. **이미지 추출**: 장면 변화 감지(Scene Detection) 방식으로 시각 자료 전환 시점 자동 포착
10. **이미지 확인**: 이미지 추출 시 작업폴더 유지, 사용자 확인 후 삭제 (시간 수정/재추출 가능)
11. **이미지 캡션**: 타임스탬프는 YouTube 링크로 연결하여 클릭 시 해당 시간으로 이동
