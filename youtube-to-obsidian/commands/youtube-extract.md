...
##### 6.8.3 영상 다운로드 및 프레임 추출

```bash
# 필요한 구간만 영상 다운로드 (시작~마지막 키프레임+10초)
yt-dlp -f "best[height<=720]" --download-sections "*0:00-<END_TIME>" -o "$WORK_DIR/video.mp4" "<YouTube_URL>"
...
8. **이미지 추출**: 핵심 장면 분석 후 통보하고 진행 (확인 대기 없음)
9. **이미지 캡션**: 타임스탬프는 YouTube 링크로 연결하여 클릭 시 해당 시간으로 이동
