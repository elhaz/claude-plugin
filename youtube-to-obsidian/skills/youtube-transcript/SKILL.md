...
이 명령어는 자막 다운로드부터 마크다운 변환까지 모든 과정을 자동화한다.
...
2. **마크다운 변환**
   ```bash
   PYTHONIOENCODING=utf-8 uv run ${CLAUDE_PLUGIN_ROOT}/scripts/vtt_to_markdown.py "VTT파일경로" --delete-vtt
   ```

3. **영어 자막 번역** (필요시)
   - Claude에게 생성된 마크다운 파일 번역 요청

4. **Obsidian 스타일로 재구성** (핵심 단계)
