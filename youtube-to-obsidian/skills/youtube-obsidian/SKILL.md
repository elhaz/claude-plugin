...
> [!important] 작업 원칙
> 1. **임시 폴더 사용**: Obsidian이 파일을 자동 수정하는 것을 방지하기 위해, 모든 중간 작업은 임시 폴더에서 수행하고, 최종 완성된 파일만 vault로 이동
> 2. **채널 폴더 우선 배치**: 같은 채널의 문서가 모여 있는 폴더가 이미 있으면 그곳에 배치한다. `00_Inbox/` 는 그런 폴더가 없을 때의 폴백이다
> 3. **병렬 처리**: 여러 URL이 제공되면 각 URL을 별도의 서브에이전트에서 병렬로 처리
...
6. **배치 위치 결정 및 최종 파일 이동**

   채널명으로 vault를 검색(Grep)해 같은 채널의 기존 문서 폴더를 찾는다.

   - **찾은 경우**: 그 폴더로 이동한다. 이어서 ① 같은 폴더 기존 문서를 읽고 구조 컨벤션(브레드크럼, 요약 콜아웃, 목차 등)을 맞추고, ② 폴더명과 같은 이름의 채널 인덱스 문서가 있으면 영상 목록에 링크를 추가한다. (`.base` 로 자동집계하는 폴더면 링크 추가 생략)
   - **못 찾은 경우**: `00_Inbox/` 로 이동한다.

   ```bash
   MD_FILE=$(ls -t "$WORK_DIR"/*.md | head -1)
   mv "$MD_FILE" "$DEST/"   # DEST = 채널 전용 폴더 또는 00_Inbox
   rm -rf "$WORK_DIR"       # 이미지를 추출한 경우엔 사용자 확인 후 삭제
   ```
